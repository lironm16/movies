...
class HomeViewModel {
    ...
    @Published private(set) var screenMode : ScreenMode = .all
    @Published private(set) var fetchStatus : FetchStatus = .ready
    

    func getMoviesData(search : String? = nil) {
        NetworkManager.shared.getData(endpoint: .movies, search: search,type: MoviesResult.self, decoder:JSONMoviesDecoder())
            .sink { [weak self] completion in
                switch completion {
                case .failure(let err):
                    print("Error is \(err.localizedDescription)")
                    self?.fetchStatus = .error(err.localizedDescription)
                case .finished:
                    print("Finished")
                }
            }
            receiveValue: { [weak self] moviessData in
                self?.movies = moviessData.movies
            }
...
class MoviesViewModel {
   ...
   // private(set) var movieRowViewModels = [String:MovieViewModel]()
    private(set) var moviesDict = [String:Movie]()
    private(set) var genres = [String]()
    private(set) var filteredMovies = [Movie]()
    
    @Published var showLoading: Bool = false
    var statusSubject: CurrentValueSubject<FetchStatus, Never> = .init(.ready)
    @Published private(set) var status : FetchStatus = .ready
    private let moviesAPIClient : MoviesAPIClientProtocol

    var stream = Set<AnyCancellable>()

    init(moviesAPIClient: MoviesAPIClientProtocol) {
...
//                if movieRowViewModels[genre] == nil {
//                    movieRowViewModels[genre] = MovieViewModel(viewModel: self)
//                }
//                
//                movieRowViewModels[genre]?.movies.append(movie.id)
//            }
//        }
//        genres = Array(movieRowViewModels.keys)
//        self.status = .finished
    }
    
    func downloadAllMovies() {
//        status = .ongoing
//        MoviesAPIClient.shared.fetchMovies().sink { [weak self] _ in
//
...
//        }.store(in: &stream)
    ...
    func fetchMovies(by search: String) {
        
//        MoviesAPIClient.shared.fetchMovies(search: search).sink { [weak self] _ in
//
//        } receiveValue: { [weak self] movies in
//            self?.filteredMovies = movies
//            //self?.status = .searchResults
//        }.store(in: &stream)
//        NetworkManager.shared.fetchMovies(urlString: urlString) { [weak self] result in
//            switch result {
...
//        }
    ...
    func getMovieBy(id:String) -> Movie? {
        return moviesDict[id]
    }
    
    func loadMovieImages(for ids:ArraySlice<String>) {
        ids.forEach { id in
            if let movie = getMovieBy(id:id) {
...
//                 "imdb_rating": 8.5,
//                 "length": "1h 55min",
//                 "overview": "When Dr. Indiana Jones \u{2013} the tweed-suited professor who just happens to be a celebrated archaeologist \u{2013} is hired by the government to locate the legendary Ark of the Covenant, he finds himself up against the entire Nazi regime.",
...
//                 "imdb_rating": 8.4,
//                 "length": "1h 45min",
//                 "overview": "Despite his family\u{2019}s baffling generations-old ban on music, Miguel dreams of becoming an accomplished musician like his idol, Ernesto de la Cruz. Desperate to prove his talent, Miguel finds himself in the stunning and colorful Land of the Dead following a mysterious chain of events. Along the way, he meets charming trickster Hector, and together, they set off on an extraordinary journey to unlock the real story behind Miguel's family history.",
